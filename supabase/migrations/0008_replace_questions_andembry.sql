...
begin
  ...
  insert into public.answer_options (question_id, text, is_correct, position) values
    (v_q_id, 'אסטרוגן מגביר את השעתוק של הגן לפקטור 12 ושל הגן לקליקראין, ובמקביל מפחית את הביטוי של אנזימים המפרקים ברדיקינין (כמו ACE)', true , 0),
  ...
  insert into public.answer_options (question_id, text, is_correct, position) values
    ...
    (v_q_id, 'חוסר ב-C1-INH מאפשר הפעלה לא מבוקרת של מערכת המגע, שכן C1-INH הוא המעכב המרכזי גם של פקטור 12 פעיל וגם של פקטור 11a, מה שמציב את החולה בסטטוס של סיכון פרו-תרומבוטי מוגבר בזמן סטרס, למרות נטייתו לבצקות', true , 1),
